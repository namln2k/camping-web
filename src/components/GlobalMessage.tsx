    ...
    <div className='fixed inset-0 z-50 w-fit mx-auto h-fit pt-4 px-8 flex flex-col items-center justify-center gap-4'>
      {messages.map((message, index) => {
        ...
        return (
          <div
            key={index}
            className={`w-fit lg:w-auto lg:min-w-[800px] lg:max-w-[75%] border rounded-lg flex items-center justify-between gap-4 ${background(
              type
            )} bg-opacity-95 max-h-36 px-3 py-3 text-white`}
          >
            <div className='w-fit flex items-center gap-2'>
              {icon(type)}
              <div className='w-fit text-justify'>
                {title && (
                  <p className='text-base font-bold block w-fit'>{title}</p>
                )}
